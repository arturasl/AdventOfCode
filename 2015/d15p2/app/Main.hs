...
solve lns = maximum scoreAttempts
  where
    ...
    ingredientCountsPerAttempt = map (map sum . List.transpose) ingredientAttemps
    validIngredientCountsPerAttempt = map init $ filter (\attempt -> last attempt == 500) ingredientCountsPerAttempt
    scoreAttempts = map (product . map (max 0)) validIngredientCountsPerAttempt
