...
data EType = Shield | Poison | Recharge | HardMode deriving (Show, Eq, Ord)
...
addHardMode :: (Player, Player) -> Maybe (Player, Player)
addHardMode = addAffect 0 Effect {tp = HardMode, timeLeft = maxBound} Knight
...
applyEffect p Effect {tp = HardMode, timeLeft} =
  if timeLeft `mod` 2 == maxBound `mod` 2
    then changeStats (empty {life = -1}) p
    else p
...
applyExistingEffects p@Player {effects} = playerAfterEffects {effects = leftEffects}
  where
    ...
    leftEffects =
      filter
        (\Effect {timeLeft} -> timeLeft > 0)
        $ map (\e@Effect {timeLeft} -> e {timeLeft = pred timeLeft}) effects
...
solve :: [T.Text] -> Int
solve lns = minMana (search Ctx {minMana = 2000, searchSpace = [fromJust $ addHardMode (knight, boss)]})
  where
