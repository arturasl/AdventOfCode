...
(defn run-in-seq [memory phase-settings]
  (reduce (fn [input phase-setting]
            (exec-single-output memory [input phase-setting]))
          0
          phase-settings))
...
(defn solve [s]
  (let [program (str->program s)]
    (reduce max (map (partial run-in-seq (:memory program)) (combo/permutations (range 5))))))
